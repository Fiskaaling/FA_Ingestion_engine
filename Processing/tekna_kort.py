        ...
        if "=" in command:
            ...
            if chk_req(variable, value, v_dic):
                ...
                if renderengine == '3D_botn':
                    ...
                    ax.plot(lineData['lon'].values, lineData['lat'].values, lin_farv, linewidth=lin_std, label=lin_legend)
                ...
                if renderengine == '3D_botn':
                    ...
                    if scatter_tekst:
                        ...
                        for i in range(len(line_x)):
                            ...
                            ax.text(line_x[i]+0.001, line_y[i], lables[i], horizontalalignment='left', va = 'center', zorder=1000000)
                    else:
                        if Samla:
                ...
                pos_lon = Qdata['lon']
                pos_lat = Qdata['lat']
                v_org = Qdata['v'] / 1000  # Frá mm/s til m/s
                u_org = Qdata['u'] / 1000
                v = v_org * qskala
                u = u_org * qskala
                lon_undir = []
                ...
                print(x_undir)
                if len(x_undir):
                    q = ax.quiver(x_undir, y_undir, u_undir, v_undir, color='g', scale=1, width=0.003, headwidth=5, zorder=100, transform=ccrs_projection)
                    ax.quiverkey(q, 0.85, 0.95 - 0 * 0.03, quiverf_threshold * qskala, label='Undir ' + str(quiverf_threshold) + ' m/s', labelpos='W')  # 2.57222
                if len(x_yvir):
                    q2 = ax.quiver(x_yvir, y_yvir, u_yvir, v_yvir, color='r', scale=1, width=0.003, headwidth=5, zorder=100, transform=ccrs_projection)
                    ax.quiverkey(q2, 0.85, 0.95 - 1 * 0.03, quiverf_threshold * qskala, label='Yvir ' + str(quiverf_threshold) + ' m/s', labelpos='W')
